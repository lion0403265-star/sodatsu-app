...
import {
  ...
  TrendingUp, Download, Moon, FolderKanban, Hourglass, GripVertical,
} from "lucide-react";
...
// GitHub Pages等の実サイトではwindow.storage(Claude artifact専用API)が使えないため、
// ブラウザのlocalStorageを使う薄いラッパーに置き換えている
const storage = {
  get: async (key) => {
    try {
...
// item.status: 'inbox' | 'next' | 'project' | 'waiting' | 'calendar' | 'someday'
const DEFAULT_ITEMS = [
...
// ---------- 最長連続記録（2日ルールの猶予を反映） ----------
function longestStreak(habit, ref = todayStr(), lookbackDays = 120) {
  let max = 0;
  let run = 0;
  let misses = 0;
...
// ---------- ドラッグ並び替え共通ヘルパー ----------
function reorderIds(idsArray, draggedId, targetId) {
  const arr = idsArray.filter((id) => id !== draggedId);
  const targetIndex = arr.indexOf(targetId);
  if (targetIndex === -1) return idsArray;
...
export default function HabitApp() {
  ...
  const addItem = useCallback((patch) => {
    setItems((prev) => [...prev, { id: uid(), done: false, createdAt: todayStr(), order: Date.now(), ...patch }]);
  }, []);
...
function FocusView({ items, updateItem, setFrog, eatFrog, habits, markHabit }) {
  ...
  nextActions = [...nextActions].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));

  const handleDrop = (targetId) => {
    if (!dragId || dragId === targetId) { setDragId(null); return; }
    const newIds = reorderIds(nextActions.map((i) => i.id), dragId, targetId);
    newIds.forEach((id, idx) => updateItem(id, { order: idx }));
    setDragId(null);
  };
        ...
        <div className="space-y-2">
          {nextActions.map((t) => (
            <ItemRow
              key={t.id}
              item={t}
              onToggle={() => updateItem(t.id, { done: !t.done })}
              draggable
...
function ItemRow({ item, onToggle, showTime, draggable, onDragStart, onDragOver, onDrop }) {
  return (
    <div
      draggable={draggable}
    ...
    >
      {draggable && <GripVertical size={14} color="#C9C4B5" className="shrink-0 cursor-grab" />}
      <button
        onClick={onToggle}
...
const STATUS_LABELS = { inbox: "Inbox", next: "Next Action", project: "プロジェクト", waiting: "対応待ち", calendar: "予定", someday: "いつか" };

function ProjectProgress({ subtasks }) {
  const total = (subtasks || []).length;
  const done = (subtasks || []).filter((s) => s.done).length;
  if (total === 0) {
    return <span className="text-[10px] rounded-full px-2 py-0.5" style={{ background: "#EFEDE3", color: "#8A9284" }}>未着手</span>;
  }
  const pct = Math.round((done / total) * 100);
  return (
    <div className="flex items-center gap-1.5 shrink-0">
      <div className="rounded-full overflow-hidden" style={{ width: 44, height: 6, background: "#EFEDE3" }}>
        <div style={{ width: `${pct}%`, height: "100%", background: "#6E9A5E" }} />
      </div>
      <span className="text-[10px]" style={{ color: "#8A9284" }}>{done}/{total}</span>
    </div>
  );
}
...
function InboxView({ items, addInboxItem, updateItem, deleteItem, addItem, inboxCount, onStartWizard, onStartReview }) {
  ...
  const list = segment === "all" ? items : items.filter((i) => i.status === segment);
    ...
    <div className="px-5 space-y-4">
      ...
      <div className="space-y-2">
        ...
        {list.map((it) => {
          ...
          const isStale = it.status === "waiting" && days >= 7;
          return (
            <div
              key={it.id}
              onClick={() => it.status === "project" && setOpenProjectId(it.id)}
              className="flex items-center gap-2 rounded-xl px-3 py-2.5"
              style={{ background: "#FFFFFF", border: isStale ? "1px solid #E3B29E" : "1px solid #E4E2D8", cursor: it.status === "project" ? "pointer" : "default" }}
            >
              <span className={`text-sm flex-1 ${it.done ? "line-through" : ""}`} style={{ color: it.done ? "#B4B8AA" : "#2B3328" }}>{it.title}</span>
              {segment === "all" && (
              ...
              {it.status === "project" && <ProjectProgress subtasks={it.subtasks} />}
              {it.status !== "inbox" && (
...
function ProjectDetailModal({ project, onClose, updateItem, addItem }) {
  ...
  const handleDrop = (targetId) => {
    if (!dragId || dragId === targetId) { setDragId(null); return; }
    const newIds = reorderIds(subtasks.map((s) => s.id), dragId, targetId);
    const newSubtasks = newIds.map((id) => subtasks.find((s) => s.id === id));
    updateItem(project.id, { subtasks: newSubtasks });
    setDragId(null);
  };
  ...
  return (
    <div className="fixed inset-0 flex items-end justify-center z-50" style={{ background: "rgba(43,51,40,0.4)" }}>
      <div className="w-full max-w-md rounded-t-3xl p-5 font-body max-h-[80vh] overflow-y-auto" style={{ background: "#F5F6F1" }}>
        ...
        <div className="mb-4">
          <ProjectProgress subtasks={subtasks} />
        </div>
        ...
        <div className="space-y-2">
          ...
          {subtasks.map((s) => (
            <div
              ...
              style={{ background: "#FFFFFF", border: "1px solid #E4E2D8" }}
            >
              <GripVertical size={13} color="#C9C4B5" className="shrink-0 cursor-grab" />
              <button
...
function HabitManageCard({ habit, updateHabit }) {
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: "#FFFFFF", border: "1px solid #E4E2D8" }}>
      ...
      <p className="text-[11px] mt-1" style={{ color: "#A8ACA0" }}>累計達成日数: {totalDone}日　・　過去最高 {best}日連続</p>

      <div className="flex items-center justify-between mt-3">
        <button onClick={() => setShowHeatmap((v) => !v)} className="flex items-center gap-1 text-[11px]" style={{ color: "#8A9284" }}>
          <CalendarClock size={12} /> 月間ヒートマップ
        </button>
      ...
      {pauseCount > 0 && (
        <div className="text-right mt-1">
          <button onClick={clearPause} className="text-[11px]" style={{ color: "#C97B63" }}>お休み設定をクリア</button>
        </div>
      )}
...
function HabitHeatmap({ habit }) {
  ...
  return (
    <div className="mt-3 rounded-xl p-3" style={{ background: "#F5F6F1" }}>
      <div className="flex items-center justify-between mb-2">
        <button onClick={() => shiftMonth(-1)} className="text-xs px-2" style={{ color: "#8A9284" }}>‹</button>
        <p className="text-[11px] font-display">{year}年{month}月</p>
        <button onClick={() => shiftMonth(1)} className="text-xs px-2" style={{ color: "#8A9284" }}>›</button>
      </div>
      <div className="grid grid-cols-7 gap-1">
        {cells.map((d, i) => {
          ...
          const scheduled = isScheduledOn(habit, d);
          const status = scheduled ? habit.history[d] || null : "skip";
          return <div key={d} title={d} className="aspect-square rounded" style={{ background: leafColor(status) }} />;
        })}
      </div>
      <p className="text-[10px] mt-2" style={{ color: "#A8ACA0" }}>緑=達成、黄=最小版、グレー=未達成、薄グレー=対象外の日</p>
    </div>
  );
}
...
// ================= スケジュール（月表示カレンダー＋週表示＋日ごとのタイムブロッキング） =================
...
function MonthCalendar({ calendarItems, selectedDate, onSelectDate }) {
  const [cursor, setCursor] = useState(`${selectedDate.slice(0, 7)}-01`); // 表示中の月の1日
  const year = Number(cursor.slice(0, 4));
  const month = Number(cursor.slice(5, 7)); // 1-12
